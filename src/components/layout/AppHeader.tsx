  ...
  return (
    <header className="h-11 border-b border-[var(--border-subtle)] bg-[var(--surface-sidebar)] px-3 flex items-center z-[var(--z-sticky)] transition-colors duration-[var(--duration-fast)] ease-[var(--ease-standard)]">
      ...
      <div className="flex-1 flex justify-center min-w-0 px-2 sm:px-4">
        {activeNote && (
          <nav className="flex items-center gap-0 text-[13px] min-w-0 overflow-hidden">
            {/* Vault name */}
            <button
              onClick={() => {
                if (isMobile) setOpen(true);
              }}
              className="shrink-0 hover:text-[#E4E4E4] transition-colors cursor-pointer truncate max-w-[120px] md:max-w-[160px] text-[#949494]"
            ...
            {pathParts.length > 0 && (
              <>
                <ChevronRight className="w-3.5 h-3.5 shrink-0 text-[#737373] mx-1" />
                ...
                {pathParts.slice(0, -1).map((part, i) => {
                  ...
                  return (
                    <React.Fragment key={i}>
                      {i > 0 && <ChevronRight className="w-3.5 h-3.5 shrink-0 text-[#737373] mx-1" />}
                      <button
                        onClick={() => {
                          expandFolderPath(fullPath);
                          if (isMobile) setOpen(true);
                        }}
                        className="shrink-0 hover:text-[#E4E4E4] transition-colors cursor-pointer truncate max-w-[100px] md:max-w-[140px] text-[#949494]"
                        title={fullPath}
                      >
                        {part}
                      </button>
                ...
                <button
                  ...
                  className="shrink-0 hover:text-[var(--accent-text)] transition-colors cursor-pointer truncate max-w-[140px] md:max-w-[200px] text-[#E4E4E4] font-medium"
