...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (retryAfter !== null) {
      return new Response(
        JSON.stringify({ error: 'rate_limited', error_description: 'Too many device authorization requests' }),
        {
          status: 429,
          headers: {
    ...
    try {
      ...
      const allowedFields = new Set(['client_id', 'scope']);
      ...
      const scope = params.get('scope') || 'repo read:user';
      if (scope.length > 200 || !/^[A-Za-z0-9:_ -]+$/.test(scope)) {
        return jsonError(400, 'invalid_scope', 'Invalid OAuth scope');
      }
      ...
      return jsonError(502, 'proxy_error', 'GitHub device authorization request failed');
    }
  },
};
